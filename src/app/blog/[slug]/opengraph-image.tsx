...
export async function generateMetadata({ params }: Props) {
  ...
  return {
    alt: post ? `${post.title} — Amresh Chaurasiya` : 'Blog post — Amresh Chaurasiya',
  };
}
...
export default async function BlogPostOpenGraphImage({ params }: Props) {
  ...
  if (!post) {
    return generateOgImage({
      ...
      body: 'Browse the rest of the writing archive on amreshdev.me/blog.',
      stats: [],
      showPortrait: false,
